...
if uploaded_main_data is not None and uploaded_machine_list is not None:
   ...
   if main_df is not None:
       try:
           ...
           torque_lower_whisker, torque_upper_whisker, torque_outliers = calculate_whisker_and_outliers(main_df['Calculated Torque [kNm]'])
           rpm_lower_whisker, rpm_upper_whisker, rpm_outliers = calculate_whisker_and_outliers(main_df['V13_SR_Drehz_nach_Abgl_Z'])
           
           # Anomaly detection
           main_df['Is_Anomaly'] = main_df['V13_SR_ArbDr_Z'] >= anomaly_threshold

           # Calculate M max Vg2
           def M_max_Vg2(rpm):
               return np.minimum(machine_params_mapped['M_max_Vg1'], 
           ...
           # Generate RPM values for torque curve
           ...
           fig.add_trace(go.Scatter(x=rpm_curve[rpm_curve <= elbow_rpm_cont],
                                  ...
                                  machine_params_mapped['M_cont_value']),
           ...
           fig.add_trace(go.Scatter(x=rpm_curve[rpm_curve <= elbow_rpm_max],
                                  ...
                                  machine_params_mapped['M_max_Vg1']),
           ...
           fig.add_trace(go.Scatter(x=[elbow_rpm_cont, elbow_rpm_cont], y=[0, y_max_vg2[1]],
                                  ...
                                  showlegend=False))
           fig.add_trace(go.Scatter(x=[machine_params_mapped['n1'], machine_params_mapped['n1']], 
                                  y=[0, y_max_vg2[2]],
                                  mode='lines', line=dict(color='black', width=1, dash='dash'), 
                                  showlegend=False))

           # Plot data points
           normal_data = main_df[~main_df['Is_Anomaly']]
           anomaly_data = main_df[main_df['Is_Anomaly']]
           
           torque_outlier_data = main_df[main_df['Calculated Torque [kNm]'].isin(torque_outliers)]
           rpm_outlier_data = main_df[main_df['V13_SR_Drehz_nach_Abgl_Z'].isin(rpm_outliers)]
           ...
           fig.update_layout(
               ...
               yaxis=dict(range=[0, max(60, main_df['Calculated Torque [kNm]'].max() * 1.1)]),
