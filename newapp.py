...
def process_and_plot_data(main_df, machine_params, sample_ratio, anomaly_threshold, P_max, nu, x_axis_max):
   ...
   pre_filter_stats = {
       'Revolution [1/min]': main_df['V13_SR_Drehz_nach_Abgl_Z'].describe(),
       'Working Pressure [bar]': main_df['V13_SR_ArbDr_Z'].describe()
   }
   ...
   rpm_filtered_count = len(filtered_df)
   
   filtered_df['Calculated Torque [kNm]'] = filtered_df.apply(
       lambda row: calculate_torque(row, machine_params), axis=1
   )
   
   filtered_df = filtered_df[filtered_df['Calculated Torque [kNm]'] > 0]
   torque_filtered_count = len(filtered_df)
   
   filtering_summary = pd.DataFrame({
       'Stage': ['Initial Data', 'After RPM Filter', 'After Torque Filter'],
       'Count': [initial_count, rpm_filtered_count, torque_filtered_count],
       'Percentage': [100, 
                     (rpm_filtered_count/initial_count)*100,
                     (torque_filtered_count/initial_count)*100]
   })
   
   post_filter_stats = {
       'Revolution [1/min]': filtered_df['V13_SR_Drehz_nach_Abgl_Z'].describe(),
       'Working Pressure [bar]': filtered_df['V13_SR_ArbDr_Z'].describe(),
       'Calculated Torque [kNm]': filtered_df['Calculated Torque [kNm]'].describe()
   }
   
   plot_df = filtered_df.sample(frac=sample_ratio)
   ...
   return fig, filtered_df, filtering_summary, pre_filter_stats, post_filter_stats
...
if uploaded_main_data and uploaded_machine_list:
   ...
   try:
       ...
       fig, processed_df, filtering_summary, pre_filter_stats, post_filter_stats = process_and_plot_data(
           main_df, machine_params, sample_ratio, 
           anomaly_threshold, P_max, nu, x_axis_max
       )
       
       st.subheader("Data Filtering Summary")
       st.dataframe(filtering_summary.style.format({
           'Percentage': '{:.2f}%'
       }))
       
       col1, col2 = st.columns(2)
       with col1:
           st.subheader("Pre-Filter Statistics")
           for key, value in pre_filter_stats.items():
               st.write(f"**{key}**")
               st.dataframe(value.to_frame())
       
       with col2:
           st.subheader("Post-Filter Statistics")
           for key, value in post_filter_stats.items():
               st.write(f"**{key}**")
               st.dataframe(value.to_frame())
