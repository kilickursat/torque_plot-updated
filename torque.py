...
# Define possible column names for various sensors (pressure, revolution, etc.)
sensor_column_map = {
    "pressure": ["Working pressure [bar]", "AzV.V13_SR_ArbDr_Z | DB 60.DBD 26", "Pression [bar]", "Presión [bar]"],
    "revolution": ["Revolution [rpm]", "AzV.V13_SR_Drehz_nach_Abgl_Z | DB 60.DBD 30", "Vitesse [rpm]", "Revoluciones [rpm]"]
}
...
def main():
    ...
    if raw_data_file is not None:
        ...
        # Fetch the relevant columns using the mapping
        pressure_col = get_column(df, "pressure")
        revolution_col = get_column(df, "revolution")
